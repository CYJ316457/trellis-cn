...
If `.trellis/config.yaml` explicitly enables `brainstorm.mode: external`, you may let `.trellis/scripts/brainstorm_runner.py` assist with draft questions / options / PRD scaffolding first. If external config is incomplete or the provider fails, immediately fall back to the normal native brainstorm flow.
