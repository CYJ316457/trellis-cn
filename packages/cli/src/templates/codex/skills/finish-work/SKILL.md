...
Wrap up the current session: archive the active task (and any other completed-but-unarchived tasks the user wants to clean up) and record the session journal. Code commits are NOT done here 鈥?those happen in workflow Phase 3.4 before you invoke this skill.
...
Filter out paths under `.trellis/workspace/` and `.trellis/tasks/` 鈥?those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
- `$finish-work` 鈥?this skill, survey + archive + record session
